...
out[key.var] .= var
```
where `var` is a variable of interest (annotated with a `:`).  `key.var` is an integer or an array of integers (generated by `makekey`). The user has allocated `out`, typicaly as an array of `Float64`. For `EspyInsideFunction` to work Julia must be able to map `var` onto a full array, and to convert the elements of `var`, to `Float64`.
