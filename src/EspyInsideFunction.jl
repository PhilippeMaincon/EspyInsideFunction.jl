...
# manipulation of data-strutures representing Julia expressions (::Expr)
using Printf
...
and the ressults are wanted as a vector (one element for each cycle of the loop).  Each element of the vector
shall be a `type` (a structure) with a field `material`, because a function of that name is called in the
for loop.  Within that function, a variable `a` is to be retrived.
...
`key` is a datastructure generated by [`makekey`](@ref) based on a [`@request`](@ref).
